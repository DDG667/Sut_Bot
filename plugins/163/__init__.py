...
async def gn(name, l) -> None:
    ...
    ImageDraw.Draw(im).text(
        (32, 32), j, font=ImageFont.truetype(f"{current_path}font.ttf", 256)
    )
    buf = io.BytesIO()
    im.save(buf, "png")
    buf.seek(0)
    await t163.send(MessageSegment.image(buf.read()))
    buf.close()
...
@t163.got("name", prompt="能不能告诉Sut你想听的音乐的名字?")
...
async def m4(event: GroupMessageEvent) -> None:
    try:
        ...
        try:
            ...
            await t163.finish(
                Message("Sut无法获取此音乐的数据，请检查是否需要VIP才能播放，或者联系维护人员[CQ:face,id=106]")
            )
        except NetworkError:
...
@c163.got("name", prompt="能不能告诉Sut音乐的名字?")
