...
async def handle_first_recei():
    await abc.finish(
        Message(
            "扫码入群[CQ:face,id=324]：[CQ:image,file=d47c494d2e86927670c8ce7d804bb201.image,subType=0,url=https://gchat.qpic.cn/gchatpic_new/3580426231/582489481-2182453240-D47C494D2E86927670C8CE7D804BB201/0?term=3&amp;is_origin=1]"
        )
    )
...
async def h():
    async with aiohttp.ClientSession() as r:
        msg = await (await r.get("https://api.lklblog.cn/api/qwxh.php")).text()
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    for i in await bot.get_group_member_list(group_id=gid):
        ...
        c = loads(
            str(
                await bot.get_stranger_info(user_id=i["user_id"], no_cache=True)
            ).replace("'", '"')
        )
        if b % 20 == 0:
            async with aiohttp.ClientSession() as r:
            ...
            img = await (await r.get(await a.text())).read()
...
async def _(event: GroupMessageEvent):
    ...
    a = int(event.get_plaintext().split(" ")[-1])
    for _ in range(a):
        await a7.send(Message(f"[CQ:poke,qq={uid}]"))
    with open(current_path + choice(["1.jpg", "2.png"]), "rb") as a:
        await a7.finish(MessageSegment.image(a.read()))
...
baike = on_command("百度百科")


@baike.handle()
async def _(e: GroupMessageEvent):
    a = e.get_plaintext().replace("百度百科 ", "")
    await search(baike, a)


# bao=on_command('报题号',permission=SUPERUSER)
# @bao.handle()
# async def _():
#     for i in range(1,100):
#         await bao.send(str(i))
#     await bao.finish('以上就是我不会的题了')
