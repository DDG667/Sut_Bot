...
async def c(m):
    async with aiohttp.ClientSession() as r:
        a = await r.get("http://api.qingyunke.com/api.php?key=free&appid=0&msg=" + m)
        msg = loads(await a.text())["content"]
    if "{face" in msg:
        a = msg.split("}")[0]
        a = a.split(":")[-1]
        msg = f"[CQ:face,id={a}]" + msg.split("}")[-1]
    if "菲菲" in msg:
        msg = msg.replace("菲菲", "Sut")
    if "周超辉" in msg:
...
chat = on_command("", priority=50,permission=SUPERUSER)
...
async def translate(word, a="zh", b="en"):
    ...
    async with aiohttp.ClientSession() as r:
        a = await r.get(
            f"https://fanyi-api.baidu.com/api/trans/vip/translate?q={word}&from={b}&to={a}&appid={aid}&salt={rint}&sign={sign}"
        )
    try:
        w = loads(await a.text())["trans_result"][0]["dst"]
    except KeyError:
        w = word
    finally:
        return w
...
async def chat_handle(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    async with aiohttp.ClientSession() as r:
        try:
            a = await r.get(f"https://api.wer.plus/api/min?t={args.extract_plain_text()}")
        except:
            a=q
            
    if (
        loads(await a.text()).get('num',0) > 0
        and (event.group_id == 551099482)
        or "你妈" in args.extract_plain_text()
    ):
        try:
            await bot.delete_msg(message_id=event.message_id)
            await bot.set_group_ban(
                group_id=event.group_id, user_id=event.user_id, duration=180
            )
    ...
    if 686221880 != 686221881:
        if 5 == 582489481:
            ...
            await chat.finish()
    if (
        "http:" or "https:" or ".org" or "www." or ".cn" or ".com" or ".xyz" or ".onion"
    ...
    if len(args.extract_plain_text()) > 256:
        ...
        await chat.finish()
    if (
        is_contain_chinese(args.extract_plain_text())
        or "help" in args.extract_plain_text()
    ):
...
async def gou():
    async with aiohttp.ClientSession() as a:
        msg = await a.get("https://api.oick.cn/dog/api.php")
...
async def yu():
    async with aiohttp.ClientSession() as a:
        msg = await a.get(" https://api.oick.cn/yulu/api.php")
    await yulu.finish(await msg.text())
...
async def du():
    async with aiohttp.ClientSession() as a:
        msg = await a.get("https://api.oick.cn/dutang/api.php")
    await dutang.finish(await msg.text())
...
async def cha(event: GroupMessageEvent):
    ...
    async with aiohttp.ClientSession() as a:
        info = loads(
            await (await a.get(f"http://ip-api.com/json/{ip}?lang=zh-CN")).text()
        )
        ...
        img = await (
            await a.get(
                loads(
                    await (
                        await a.get(
                            f"https://api.uomg.com/api/rand.avatar?sort={sex}&format=json"
                        )
                    ).text()
                )["imgurl"]
            )
...
async def tuw():
    async with aiohttp.ClientSession() as a:
        msg = await (
            await a.get("https://api.uomg.com/api/rand.qinghua?format=text")
        ).text()
    await tu.finish(msg)
...
async def sho(event: GroupMessageEvent):
    ...
    async with aiohttp.ClientSession() as a:
        json = loads(
            await (
                await a.get(f"https://api.uomg.com/api/long2dwz?dwzapi=tcn&url={url}")
            ).text()
        )
...
async def qrc(event: GroupMessageEvent):
    ...
    async with aiohttp.ClientSession() as a:
        qrco = await (
            await a.get(f"https://api.oick.cn/qrcode/api.php?text={url}&size=4096")
        ).read()
...
async def wea(event: GroupMessageEvent):
    ...
    async with aiohttp.ClientSession() as a:
        da = loads(await (await a.get(url, verify=False)).text())
    if da["code"] == 200:
        data1 = f"          {area}天气"
        for data in da["data"]:
        ...
        await weat.finish("请告诉Sut你要查哪个地区的天气，OK?")
...
async def hitokoto(
    ...
    if choice(a):
        async with aiohttp.ClientSession() as a:
            m = loads(
                await (await a.get("https://v1.hitokoto.cn?c=a&c=b&c=c&c=d&c=h")).text()
            )
        if m["from_who"] != None:
            msg = f"{m['hitokoto']}\n——《{m['from']}》{m['from_who']}"
        else:
        ...
        async with aiohttp.ClientSession() as a:
            msg = await (await a.get("https://api.oick.cn/yiyan/api.php")).text()
...
async def yiju_handle():
    async with aiohttp.ClientSession() as a:
        await yiju.finish(
            await (
                await a.get(
                    "http://api.yanxi520.cn/api/xljtwr.php?charset=utf-8http://api.yanxi520.cn/api/xljtwr.php?encode=txt"
                )
            ).text()
        )
...
async def _():
    async with aiohttp.ClientSession() as a:
        json = loads(
            await (
                await a.get(
                    "https://www.yuanxiapi.cn/api/img/?type=dongman&format=json"
                )
            ).text()
        )
    if json["code"] == 200:
        async with aiohttp.ClientSession() as a:
            data = await (await a.get(json["imgurl"])).read()
        await dongman.finish(MessageSegment.image(data))
    else:
        await dongman.finish(Message("Get到了但状态码不是200，Sut无能为力[CQ:face,id=106]"))
...
async def _():
    async with aiohttp.ClientSession() as b:
        a = await b.get("https://meiriyiwen.com/")
    t = findall("<h1>(.*)</h1>", await a.text())[0]
    w = findall('<p class="article_author"><span>(.*)</span></p>', await a.text())[0]
    cl = findall("<p>(.*)</p>", await a.text())
    c = ""
    for i in cl:
        c += "\n" * 2 + " " * 4 + i
    c = " " * 6 + t + "\n" + " " * 8 + w + c
